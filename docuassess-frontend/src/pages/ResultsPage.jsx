...
export default function ResultsPage() {
  ...
  const { results, fileMeta, selectedTypes } = useAppContext();
  ...
  // Get the question types that have data
  const questionTypes = Object.keys(questions || {}).filter(
    (key) => Array.isArray(questions[key]) && questions[key].length > 0
  );

  const totalQuestions = questionTypes.reduce(
    (sum, key) => sum + questions[key].length, 0
  );
    ...
    <div className="animate-fade-in">
      ...
      <div style={{ marginBottom: '32px' }}>
        ...
        <p style={{ fontSize: '14px', color: 'var(--color-text-secondary)' }}>
          {totalQuestions} questions from <strong>{fileMeta?.originalName || 'your document'}</strong>
        </p>
      </div>
      ...
      {meta && (
        ...
        >
          {[
            ...
            meta.validation && meta.validation.invalidCount > 0 && {
              label: 'Invalid',
            ...
            .map((item) => (
              <div
                ...
                style={{
                  ...
                  background: item.warn ? 'var(--color-gold-muted)' : 'var(--color-surface-alt)',
                  border: `1px solid ${item.warn ? 'rgba(212, 160, 23, 0.3)' : 'var(--color-border)'}`,
                  fontSize: '13px',
                }}
              >
                <span style={{ color: 'var(--color-text-muted)', marginRight: '6px' }}>{item.label}</span>
                <strong style={{ color: item.warn ? 'var(--color-gold)' : 'var(--color-text)' }}>{item.value}</strong>
              </div>
            ))}
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {questionTypes.length > 0 ? (
          ...
          >
            <p style={{ fontSize: '16px', color: 'var(--color-text-muted)', marginBottom: '8px' }}>
